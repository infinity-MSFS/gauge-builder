...
const lit0: BoundValue = { type: "Literal", value: 0 };

// ─── Deep element helpers ─────────────────────────────────────────────

/** Recursively find an element by id, returning it plus the accumulated
 *  translation offset of all its ancestor groups (in scene space). */
function findElementByIdDeep(
	elements: SceneElement[],
	id: string,
	vars: Map<string, number>,
	ax = 0,
	ay = 0,
): { el: SceneElement; ox: number; oy: number } | null {
	for (const el of elements) {
		if (el.id === id) return { el, ox: ax, oy: ay };
		if (el.kind.type === "Group") {
			const tx = ax + rv(el.kind.translate_x ?? lit0, vars);
			const ty = ay + rv(el.kind.translate_y ?? lit0, vars);
			const found = findElementByIdDeep(el.kind.children, id, vars, tx, ty);
...
/** Recursively update a single element's kind anywhere in the scene tree. */
function updateElementInTree(
	...
	return elements.map((el) => {
		if (el.id === id) return { ...el, kind };
		if (el.kind.type === "Group") {
			return {
				...el,
				kind: {
					...el.kind,
					children: updateElementInTree(el.kind.children, id, kind),
				},
			};
		}
		return el;
...
function getElementBBox(
	...
	switch (k.type) {
		...
		case "Group": {
			// Return the union of children bboxes shifted by the group's own translation,
			// giving the group's bbox in its parent coordinate space.
			const tx = rv(k.translate_x ?? lit0, vars);
			const ty = rv(k.translate_y ?? lit0, vars);
			let minX = Infinity,
				minY = Infinity,
				maxX = -Infinity,
				maxY = -Infinity;
			for (const child of k.children) {
				const cb = getElementBBox(child, vars);
				if (!cb) continue;
			...
			if (!Number.isFinite(minX)) return { x: tx, y: ty, w: 20, h: 20 };
			return { x: minX + tx, y: minY + ty, w: maxX - minX, h: maxY - minY };
		}
		default:
			return null;
	}
...
function moveElementKind(
	...
	switch (kind.type) {
		...
		case "Group":
			return {
				...kind,
				translate_x: litAdd(kind.translate_x ?? lit0, dx),
				translate_y: litAdd(kind.translate_y ?? lit0, dy),
			};
		default:
			return kind;
	}
...
function resizeElementKind(
	...
	switch (kind.type) {
		...
		case "Line": {
			if (handle === "tl")
				return { ...kind, x1: litAdd(kind.x1, dx), y1: litAdd(kind.y1, dy) };
			if (handle === "br")
				return { ...kind, x2: litAdd(kind.x2, dx), y2: litAdd(kind.y2, dy) };
			return kind;
		}
		default:
			return kind;
...
function getHandles(bbox: BBox, vp: Viewport) {
	...
	const p = (sx: number, sy: number) => ({
		sx: sx * vp.zoom + vp.panX,
		sy: sy * vp.zoom + vp.panY,
	});
	return [
...
function drawElement(
	...
	switch (k.type) {
		...
		case "Group": {
			const opacity = rv(k.opacity ?? { type: "Literal", value: 1 }, vars);
			const tx = rv(k.translate_x ?? { type: "Literal", value: 0 }, vars);
			const ty = rv(k.translate_y ?? { type: "Literal", value: 0 }, vars);
			const rot = rv(k.rotate ?? { type: "Literal", value: 0 }, vars);
			const sx = rv(k.scale_x ?? { type: "Literal", value: 1 }, vars);
			const sy = rv(k.scale_y ?? { type: "Literal", value: 1 }, vars);
			...
			// Apply group transform once — moving the parent slides the whole array without deformation
			ctx.translate(tx, ty);
			...
			if (!k.array_modifier) {
				...
				// Radial — cx/cy are in the group's local space
...
export default function Canvas() {
	...
	const draw = () => {
		...
		for (const el of scene.elements) {
			...
			const overrideKind =
				localKindRef.current !== null && el.id === selectedId
					? localKindRef.current
					: null;
			drawElement(
				ctx,
				overrideKind ? { ...el, kind: overrideKind } : el,
				varMap,
			);
		}
		...
		for (const img of refImages) {
			...
			if (!el) continue;
			const live =
				localImgRef.current?.id === img.id ? localImgRef.current : img;
			ctx.save();
		...
		if (selectedId) {
			...
			if (refImg) {
				const live =
					localImgRef.current?.id === refImg.id ? localImgRef.current : refImg;
	...
	const hitElement = (sx: number, sy: number): string | null => {
		...
		function hitInList(
			...
			for (let i = elements.length - 1; i >= 0; i--) {
				...
				if (!el.visible) continue;
				if (el.kind.type === "Group") {
					const tx = ox + rv(el.kind.translate_x ?? lit0, varMap);
					const ty = oy + rv(el.kind.translate_y ?? lit0, varMap);
					// Prefer hitting a specific child over the group container
					const childHit = hitInList(el.kind.children, tx, ty);
		...
		return hitInList(sceneRef.current.elements, 0, 0);
	};
	...
	const getBBoxForSelected = (id: string): BBox | null => {
		...
		const found = findElementByIdDeep(sceneRef.current.elements, id, varMap);
		if (!found) return null;
		const bbox = getElementBBox(found.el, varMap);
		if (!bbox) return null;
		return { ...bbox, x: bbox.x + found.ox, y: bbox.y + found.oy };
	};
	...
	const updateHoverCursor = (
		...
		if (hitElement(sx, sy) || hitRefImage(sx, sy)) {
			setCursor("pointer");
			return;
		}
		setCursor("default");
	...
	const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
		...
		if (selId) {
			...
			if (refImg && !refImg.locked) {
				...
				if (
					sx >= refImg.x &&
					sx <= refImg.x + refImg.w &&
					sy >= refImg.y &&
					sy <= refImg.y + refImg.h
				) {
					dragRef.current = {
						t: "move-img",
						id: selId,
		...
		if (elHit) {
			...
			const found = findElementByIdDeep(
	...
	globalMoveRef.current = (e: MouseEvent) => {
		...
		if (drag.t === "pan") {
			vpRef.current = {
				...vp,
				panX: drag.px0 + (mx - drag.sx0),
				panY: drag.py0 + (my - drag.sy0),
			};
			draw();
			return;
		}
		if (drag.t === "move-el") {
			localKindRef.current = moveElementKind(
				drag.k0,
				sx - drag.sx0,
				sy - drag.sy0,
			);
		...
		if (drag.t === "resize-el") {
			localKindRef.current = resizeElementKind(
				drag.k0,
				drag.h,
				sx - drag.sx0,
				sy - drag.sy0,
		...
		if (drag.t === "move-img") {
			...
			localImgRef.current = {
				...img,
				x: drag.x0 + (sx - drag.sx0),
				y: drag.y0 + (sy - drag.sy0),
			};
			draw();
			return;
		}
		if (drag.t === "resize-img") {
			...
			let dw = 0;
			if (drag.h === "br" || drag.h === "tr") {
				dw = ddx;
			} else if (drag.h === "bl" || drag.h === "tl") {
				dw = -ddx;
			}
	...
	useEffect(() => {
		const onPaste = (e: ClipboardEvent) => {
			...
			loadRefImageFromClipboard(items)
				.then((id) => {
					if (id) forceUpdate();
				})
				.catch((err) => {
